(function($) {
                    ...
                    if(!response.formValid){
                        if (response.action == "redirect") {
                            if (opt.redirect) {
                                ...
                                var target = (response.target == '_blank')?'_blank':'_self';
                        ...
                        if (typeof response.errors != "undefined") {
                            ...
                            if (opt.errorPosition == "field") {
                                for (var i = 0; i < keys.length; i++) {
                                    ...
                                    if (helpBlockContainer.length == 0 || key == "from_date") {
                                        helpBlockContainer = $(grp);
                                    }
                                    var helpBlockClass = (key == "from_date")?'down-help-block':'';
                                    helpBlockContainer.append(`<div class="help-block help-block-error ${helpBlockClass}">${response.errors[keys[i]]} </div>`);
    ...
    $.showErrors = function(object) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (helpBlockContainer.length == 0 || keys[i] == "from_date") {
                helpBlockContainer = $(grp);
            }

            var helpBlockClass = (keys[i] == "from_date")?'down-help-block':'';

            helpBlockContainer.append(`<div class="${helpBlockClass} help-block help-block-error">${object[keys[i]]}</div>`);
            $(grp).addClass("has-error");
        }
    }
})(jQuery);
